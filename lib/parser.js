...
var PROPERTIES = [GRAMMAR.FEEDBACK, GRAMMAR.NAME, GRAMMAR.NOTE, GRAMMAR.PAINT, GRAMMAR.PROGRAM, GRAMMAR.USAGE];
...
var convertToRGB = function (color) {
    if (/((\d{1,3}) (\d{1,3}) (\d{1,3}))/g.exec(color)) {
        return color;
    }

    var values = color.slice(1, color.length).match(/.{2}/g);
...
var Parser = function () {
    this.parse = function (line) {
        ...
        if (keys[0] === GRAMMAR.FEEDBACK || keys[0] === GRAMMAR.NOTE || keys[0] === GRAMMAR.PAINT || keys[0] === GRAMMAR.PROGRAM || keys[0] === GRAMMAR.USAGE) {
            var propertyName = keys[0];
            var propertyValue = createPropertyValue(keys);

            if (propertyName === GRAMMAR.PAINT) {
                propertyValue = convertToRGB(propertyValue);
            }
