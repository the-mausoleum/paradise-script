...
var createTernaryStatement = function (cond, tExpression, fExpression) {
    return cond + pad(CHAR.QUESTION) + tExpression + pad(CHAR.COLON) + fExpression;
};

var createUndefinedCheck = function (variable, tExpression, fExpression) {
    return createTernaryStatement('typeof ' + variable + pad(CHAR.EQUALITY) + CHAR.QUOTE + EXPRESSION.UNDEFINED + CHAR.QUOTE, tExpression, fExpression);
};
...
var Parser = function () {
    this.parse = function (line) {
        ...
        if (keys[0] === GRAMMAR.CREATE_CLOSE) {
            ...
            statement += createIndent() + pad(EXPRESSION.RETURN, true) + CHAR.BRACE_OPEN + CHAR.NEWLINE;
            ...
            for (var i in properties) {
                statement += createIndent() + properties[i] + pad(CHAR.COLON, true) + createUndefinedCheck(createPropertyName(currentVessel, properties[i]), CHAR.QUOTE + CHAR.QUOTE, currentVessel + CHAR.UNDERSCORE + properties[i]) + CHAR.COMMA + CHAR.NEWLINE;
            }
