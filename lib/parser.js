...
var generateTernaryStatement = function (cond, tExpression, fExpression) {
    return cond + ' ? ' + tExpression + ' : ' + fExpression;
};

var generateUndefinedCheck = function (variable, tExpression, fExpression) {
    return generateTernaryStatement('typeof ' + variable + ' === ' + CHAR.QUOTE + 'undefined' + CHAR.QUOTE, tExpression, fExpression);
};
...
var Parser = function () {
    this.parse = function (line) {
        ...
        if (keys[0] === GRAMMAR.CREATE_START) {
            ...
            if (indentLevel === 0) {
                statement += generateIndent() + '(function () {' + CHAR.NEWLINE;
            } else {
            ...
            statement += generateIndent() + 'var ' + generatePropertyName(currentVessel, GRAMMAR.NAME) + ' = ' + CHAR.QUOTE + vesselName + CHAR.QUOTE + CHAR.SEMICOLON + CHAR.NEWLINE;
        }
        ...
        if (keys[0] === GRAMMAR.CREATE_END) {
            ...
            for (var i in properties) {
                statement += generateIndent() + properties[i] + ': ' + generateUndefinedCheck(generatePropertyName(currentVessel, properties[i]), CHAR.QUOTE + CHAR.QUOTE, currentVessel + '_' + properties[i]) + CHAR.NEWLINE;
            }
            ...
            for (var i in children[currentVessel]) {
                statement += generateIndent() + children[currentVessel][i] + CHAR.COMMA + CHAR.NEWLINE;
            }
        ...
        if (keys[0] === GRAMMAR.FEEDBACK || keys[0] === GRAMMAR.NOTE || keys[0] === GRAMMAR.PROGRAM || keys[0] === GRAMMAR.USAGE) {
            ...
            statement += generateIndent() + 'var ' + generatePropertyName(currentVessel, propertyName) + ' = ' + CHAR.QUOTE + propertyValue + CHAR.QUOTE + CHAR.SEMICOLON + CHAR.NEWLINE;
        }
